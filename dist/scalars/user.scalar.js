...
const firstNameValidation = (value) => {
    if (typeof value !== 'string')
        throw new GraphQLError(`Value is not string : ${value}`);
    if (value.length < 2)
        throw new GraphQLError(`Value is too short : ${value}`);
    if (value.length > 12)
        throw new GraphQLError(`Value is too long : ${value}`);
    return value;
};
...
const passwordValidation = (value) => {
    if (typeof value !== 'string')
        throw new GraphQLError(`Value is not string : ${value}`);
    if (value.length < 8)
        throw new GraphQLError(`Value is too short, atleast 8 character required : ${value}`);
    if (value.length > 14)
        throw new GraphQLError(`Value is too long, maximum 14 character allowed : ${value}`);
    if (value.search(/[a-z]/i) < 0)
        throw new GraphQLError(`Value must contain atleast one letter : ${value}`);
    if (value.search(/[0-9]/) < 0)
        throw new GraphQLError(`Value must contain atleast one digit : ${value}`);
    return value;
};
